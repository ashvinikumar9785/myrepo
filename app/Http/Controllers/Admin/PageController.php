...
class PageController extends Controller
{
    ...
    public function add(Request $request,$id=null){
        if($id){
            $title = "Edit Page";
            $breadcrumbs = [
                ['name'=>'Pages','relation'=>'link','url'=>route('admin.pages.index')],
    ...
    {
        $pages = Page::select(['id', 'title', 'slug', 'status'])->get();

        return DataTables::of($pages)
            ->addColumn('action', function ($page) {
                return '<a href="'.route('admin.pages.add',$page->id).'" class="btn btn-xs btn-primary"><i class="fas fa-edit"></i> Edit</a>&nbsp;<a href="'.route('admin.pages.view',$page->slug).'" class="btn btn-xs btn-primary"><i class="fas fa-eye"></i> View</a>';
            })
            ->editColumn('status',function($page){
                return Helper::getStatus($page->status,$page->id,route('admin.pages.status'));
            })   
    ...
    public function status(Request $request)
    {
        ...
        $row->save(); 
        return Helper::getStatus($row->status,$id,route('admin.pages.status')); 
    } 
